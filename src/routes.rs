...
async fn save_mock(data: web::Json<MockAPI>, state: web::Data<AppState>) -> impl Responder {
    ...
    {
        let mut handlebars = HANDLEBARS.lock().unwrap();
        if let Err(e) = handlebars.register_template_string(&template_name, &mock.response) {
            eprintln!("Failed to compile template: {}", e);
            return HttpResponse::InternalServerError().json("Failed to compile template");
        }
    }
...
async fn update_mock(
    ...
    let template_name = mock_id.to_string();
    {
        ...
        if let Err(e) = handlebars.register_template_string(&template_name, &updated_mock.response)
        {
            eprintln!("Failed to compile template: {}", e);
            return HttpResponse::InternalServerError().json("Failed to compile template");
        }
    ...
    if let Some(mock_id) = state.api_name_to_id.get(&api_name) {
        if let Some(mock) = state.mocks.get(&mock_id.value()) {
            ...
            if req.method().as_str().eq_ignore_ascii_case(&mock.method) {
                ...
                // Render the response using the precompiled template
                let template_name = mock.id.unwrap().to_string();
                let rendered = {
                    let handlebars = HANDLEBARS.lock().unwrap();
